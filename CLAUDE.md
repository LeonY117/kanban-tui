...
- `/` filters the board in place, live, over every readable field; terms are ANDed and `#tag` matches tags only. Matching is case-insensitive **substring**, so `#cli` also selects `#client` — the completion counts say so. `ctrl+g` widens to every active board, whose cards arrive badged and read-only; `enter` follows one home. Session-only, cleared on board switch.
