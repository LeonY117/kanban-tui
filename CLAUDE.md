...
- `#` opens the board's tags as a list, from the board or the picker (`t` still works too — it's a reserved alias, so no rebind can claim it), bookended by **all tickets** and **no tags** (`-#`). Picking one writes the query you could have typed, so there is one filter with one meaning; picking replaces rather than ANDs. Any exit key leaves for the board, not back to the picker.
...
- **Every list selects on the first click and acts on a second click of the same row** — cards, boards, tags, move rows, settings, and the title / description panels, whose second click opens the editor. Clicks also reach what only the keyboard used to: the Info bar's status / assignee / tags fields, the add popup's fields, the rename form's inputs. A click inside an editor already open is a no-op, not a save-and-reopen.
